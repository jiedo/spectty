...
- **默认输入路径必须先正确**，要尽量逼近标准 Linux / xterm 终端行为
- **配置化是后续增强层**，只能做本地覆盖、宏和穿透，不能用来修复默认编码 bug
- **远端配置保持默认**，本地终端做的按键映射不应要求用户修改远端 `tmux`、`emacs` 或 shell 配置

当前硬件键盘路径大致为：

1. `TerminalMetalView` 接收 `UIKey`
2. `HardwareKeyInterpreter` 负责把 UIKit 提供的字符信息归一化成终端语义上的 `characters`
3. `TerminalSession` 把 `KeyEvent` 交给 `GhosttyTerminalEmulator.encodeKey(_:)`
4. `KeyEncoder` 根据终端模式把按键事件编码为发给远端的字节流

这条链路里，`TerminalMetalView` / `HardwareKeyInterpreter` 决定“这个按键应该被看作什么字符”，`KeyEncoder` 决定“这个字符最终该编码成什么控制序列或字节”。

### 10.4 2026-04 已完成的键盘修复

为了让 `emacs`、`tmux`、shell、readline 等标准终端程序正常工作，2026-04 这一轮修复先只处理默认输入语义，不引入配置层。

核心改动如下：

- `TerminalMetalView.swift` 中新增了 `HardwareKeyInterpreter`
  - 把外接键盘字符归一化逻辑从按键分发流程里抽成独立纯函数
  - 带 `Alt` / `Ctrl` / `Command` 的组合键会优先还原
    `charactersIgnoringModifiers` 对应的终端基字符，再补标准
    ASCII `Shift` 映射，避免把 iOS `Option` 产出的布局字符
    （如 `ƒ`、`¡`）误发到远端
- `TerminalMetalView` 现在通过 `HardwareKeyInterpreter` 统一解释外接键盘字符
- `KeyEncoder` 补齐了一批标准控制字符别名和 Meta 特殊键行为
...
- `Alt+Shift+.` → `M->`
- `Alt+Shift+5` → `M-%`
- `Ctrl+Shift+2` / `Ctrl+@` → `NUL` (`0x00`)
- `Ctrl+6` / `Ctrl+^` → `RS` (`0x1E`)
- `Ctrl+/`、`Ctrl+_`、`Ctrl+7` → `US` (`0x1F`)
- `Ctrl+8` → `DEL` (`0x7F`)
- `Alt+Backspace` → `ESC DEL`
- `Alt+Return` → `ESC CR`

这一轮修复的直接目标是：不要再让 `M-<`、`M->`、`M-%`、`C-@`、`C-/` 这类标准终端按键因为本地输入层错误而失效。

### 10.5 软件键盘辅助栏与外接键盘行为

2026-04 这一轮还明确收敛了 iOS 屏幕虚拟按键区的行为，避免“隐藏系统键盘后失去终端控制键”。

当前实现约束如下：

- 虚拟按键区由 `TerminalMetalView` 自己管理，作为视图底部的持久工具栏存在，而不是挂在 `inputAccessoryView`
- 手动隐藏系统键盘后，这个虚拟按键区仍然保持可见，方便继续使用终端控制键并扩大正文显示区域
- 当系统判断当前处于外接键盘输入模式时，这个虚拟按键区必须隐藏
- 当前工具栏为两排布局：
  - 第一排：`Esc`、`Tab`、`Ctrl`、`Shift`、方向键
  - 第二排：`Del`（forward delete，不是 backspace）、`PgUp`、`PgDn`、系统键盘显示/隐藏按钮
- `TerminalMetalView` 的终端内容区域与网格计算会为该持久工具栏预留底部空间，因此后续若修改底部 inset、键盘重叠处理或 grid 计算，必须一并检查该行为
...
- `Packages/SpecttyTerminal/Tests/SpecttyTerminalTests/KeyEncoderTests.swift`
  - 覆盖标准控制字符映射和 `Alt` 前缀编码
- `Packages/SpecttyUI/Tests/SpecttyUITests/HardwareKeyInterpreterTests.swift`
  - 覆盖外接键盘字符归一化，如 `M-<`、`M->`、`M-%`、`C-@`

需要注意：

- `SpecttyTerminal` 的 SwiftPM 单测可以直接运行
- `SpecttyUI` 已有测试 target，但当前共享 scheme 没有启用 test action，因此这部分目前主要通过“编译通过”保证没有集成错误

本轮修复用过的验证命令：

- `cd Packages/SpecttyTerminal && swift test`
- `xcodebuild build -scheme SpecttyUI -destination 'generic/platform=iOS'`
- `xcodebuild build -scheme Spectty -destination 'generic/platform=iOS' CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO CODE_SIGN_IDENTITY=''`

最后一条使用 no-signing 构建，是为了把“代码能否完整编进 App”与本机开发证书状态解耦。

### 10.7 下一步行动

后续如果继续沿终端正确性推进，优先级建议保持如下顺序：

1. 继续修标准终端语义
   - 重点关注功能键、编辑键、小键盘、`Alt+Arrow`、`Alt+Home/End`、`Shift+Insert` 这类常见终端组合
2. 扩大回归测试覆盖
   - 优先补 `KeyEncoderTests`
   - 再把 `SpecttyUI` 的测试执行路径接通，而不只是编译校验
3. 只有在默认语义稳定后，才开始本地按键配置化
   - 配置层应位于默认编码层之上
   - 配置只负责本地覆盖、宏和穿透
   - 配置不能承担“修 bug”的职责
4. 配置化落地时保持远端透明
   - 不要求修改远端 `tmux` / `emacs` / shell 配置
   - 保持“本地终端增强，远端默认不变”的原则

如果下次需要从零上下文继续这条线，建议优先阅读以下文件：

- `Packages/SpecttyUI/Sources/SpecttyUI/TerminalMetalView.swift`
- `Packages/SpecttyUI/Sources/SpecttyUI/TerminalInputAccessory.swift`
- `Packages/SpecttyTerminal/Sources/SpecttyTerminal/KeyEncoder.swift`
- `Packages/SpecttyUI/Tests/SpecttyUITests/HardwareKeyInterpreterTests.swift`
- `Packages/SpecttyTerminal/Tests/SpecttyTerminalTests/KeyEncoderTests.swift`
