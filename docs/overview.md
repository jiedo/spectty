...
- 新增 `Packages/SpecttyUI/Sources/SpecttyUI/HardwareKeyInterpreter.swift`
  - 把原本写在 `TerminalMetalView` 里的外接键盘字符归一化逻辑抽成纯函数
  - 带 `Alt` / `Ctrl` / `Command` 的组合键会优先保留布局后的可打印字符，不再过早退回 `charactersIgnoringModifiers`
- `TerminalMetalView` 现在通过 `HardwareKeyInterpreter` 解释外接键盘字符
...
- `Packages/SpecttyUI/Sources/SpecttyUI/TerminalMetalView.swift`
- `Packages/SpecttyUI/Sources/SpecttyUI/HardwareKeyInterpreter.swift`
